...
class Room extends EventEmitter
{
	...
	close()
	{
		...
		{
			console.debug('close() | stopping network throttle');

			throttle.stop({})
				.catch((error) =>
				{
					console.error(`close() | failed to stop network throttle:${error}`);
				});
		}
	}
		...
		})
	{
		...
		switch (type)
		{
			case 'webrtc':
			{
				...
				{
					...clone(config.mediasoup.webRtcTransportOptions),
			...
			case 'plain':
			{
				...
				{
					...clone(config.mediasoup.plainTransportOptions),
					rtcpMux : rtcpMux,
					comedia : comedia
				};
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		switch (request.method)
		{
			...
			case 'produce':
			{
				...
				// 	console.info('RTCP packet');
				// 	console.info(rtcpPacket.dump());
				// });
				...
				// 	// console.info('RTP packet');
				// 	// console.info(rtpPacket.dump());
