...
const config = {
  // Listening hostname.
  domain: process.env.DOMAIN || 'localhost', // Signaling settings (protoo WebSocket server and HTTP API server).
  https: {
    listenIp: '0.0.0.0', // NOTE: Don't change listenPort (client app assumes 4443).
    listenPort: process.env.PROTOO_LISTEN_PORT || 4443, // NOTE: Set your own valid certificate files.
    tls: {
  ...
  mediasoup: {
    ...
    numWorkers: Object.keys(os.cpus()).length, // mediasoup WorkerSettings.
    ...
    // mediasoup WebRtcServer options for WebRTC endpoints (mediasoup-client, libmediasoupclient).
    ...
    // mediasoup WebRtcTransport options for WebRTC endpoints (mediasoup-client, libmediasoupclient).
    ...
    webRtcTransportOptions: {
      ...
      listenInfos: [
        {
          ...
          announcedAddress: process.env.MEDIASOUP_ANNOUNCED_IP,
          portRange: {
            min: process.env.MEDIASOUP_MIN_PORT || 40000, max: process.env.MEDIASOUP_MAX_PORT || 49999
          }
        },
        {
          protocol: 'tcp',
      ...
      maxSctpMessageSize: 262144, // Additional options that are not part of WebRtcTransportOptions.
      maxIncomingBitrate: 1500000
    },
    // mediasoup PlainTransport options for legacy RTP endpoints (FFmpeg, GStreamer).
